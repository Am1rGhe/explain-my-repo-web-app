        ...
        <div className="mt-auto p-4 space-y-3 border-t border-app-border">
          ...
          {status === "unauthenticated" && (
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-lg border border-app-border bg-app-button py-2 px-3 text-sm font-medium text-app-main hover:bg-app-border transition-colors cursor-pointer"
            >
          ...
          {status === "authenticated" && session?.user && (
            <div className="space-y-2">
              <p className="text-xs text-app-muted truncate" title={session.user.email ?? undefined}>
                {session.user.name ?? session.user.email ?? "Signed in"}
              </p>
              <button
                type="button"
                onClick={() => signOut({ callbackUrl: "/" })}
                className="w-full rounded-lg border border-app-border bg-app-button py-2 px-3 text-xs font-medium text-app-muted hover:text-app-main hover:bg-app-border transition-colors cursor-pointer"
              >
                Sign out
              </button>
            </div>
